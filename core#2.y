...
%token <ival> DIGIT
%token <fval> FLOAT
%token <sval> ID ELSE IF WHILE TYPES REL_OPT OR AND NOT TRUE FALSE HASH COLON CHAR
%token <sval> INT Float DOUBLE COMMA SEMICOLON SEMISEMICOLON a STOP FOR
%token <sval> '+' '-' '*' '/' '^' '%' '\n' '=' ';'
...
startSym: Declarations HASH program HASH
		{
			s1 = $3;
			label = newLabel();
...
//****************************
Declarations :
             |Declarations datatype ID COMMA
             ;         
...
construct :    construct block
		...
		|
		FOR stat COMMA bool COMMA stat COMMA block STOP
		{
			printf("Inside FOR\n");
			puts($8);
